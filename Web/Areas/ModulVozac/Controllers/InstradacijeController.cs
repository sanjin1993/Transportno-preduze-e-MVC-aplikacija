...
using TransportnoPreduzece.Areas.ModulVozac.Models;
using TransportnoPreduzece.Data.DAL;
using TransportnoPreduzece.Data.Models;
using TransportnoPreduzece.Web;
using TransportnoPreduzece.Web.Areas.ModulDispecer.Models;

namespace TransportnoPreduzece.Areas.ModulVozac.Controllers
{
    [Authorize(Roles = "vozač")]
    //[RouteArea("ModulVozac", AreaPrefix = "")]
    public class InstradacijeController : Controller
    {

        int vozacId = Global.odabraniVozac.ZaposlenikId;
        ...
        {
            var Model = ctx.KarticaVozaci.Where(x => x.VozacId == Global.odabraniVozac.ZaposlenikId).Include(x => x.BenzinskaPumpa)
                .OrderByDescending(x => x.DatumKoristenja)

                .Select(x =>
                  new TrosakBenzinPrikaziVM()
                  {
                      DpdijeljenIznos = (double)ctx.KarticaZaposlenici.Where(y => y.ZaposlenikId == Global.odabraniVozac.ZaposlenikId).FirstOrDefault().Iznos,
                      TrenutniIznos = ctx.KarticaZaposlenici.Where(h => h.ZaposlenikId == Global.odabraniVozac.ZaposlenikId).FirstOrDefault().Kartica.TrenutniIznos,
        ...
        public JsonResult ZakljuciBenzinTrosak(TrosakBenzinEditVM model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (k.TrenutniIznos > model.UkupanIznos)
                {
                    ...
                    ctx.SaveChanges();
                    return Json(new { RedirectUrl = Url.Action("PrikaziKartice", "Instradacije", new { vozacId = model.VozacId }) });
                ...
                else
                {
                        ...
                        {
                            Message = x.Value.Errors.Select(y => y.ErrorMessage).FirstOrDefault(),
                            Name = "Trenutni iznos nije dovoljan za transakciju"
                        }).ToList();
                        return Json(new { Errors = errors });
